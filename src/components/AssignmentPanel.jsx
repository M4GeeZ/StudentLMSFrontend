...
const AssignmentPanel = () => {
  ...
  const getDeadlineStatus = (deadline) => {
    ...
    if (diff < 0)
      return { text: "Expired", color: "bg-red-500/20 text-red-300" };
    if (diff <= 2)
      return {
        text: `${diff} days left`,
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
      <div className="bg-white rounded-2xl shadow p-6 border border-slate-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          />
          <textarea
            className="input min-h-28 !text-black"
            name="description"
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between gap-3">
                      ...
                      <span
                        className={`h-fit px-3 py-1 rounded-full text-xs !text-black font-semibold ${status.color}`}
                      >
                        {status.text}
                      </span>
                    </div>
